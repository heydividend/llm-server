#!/usr/bin/env python3
"""
Harvey ML Scheduler Sanity Check v5.0 - With Recovery Notifications
Checks system health and sends email alerts for both issues and resolutions.
"""
    ...
    def save_state(self, health_score: int, alert_type: Optional[str], 
        ...
        try:
            ...
            if health_score < 80 and self.state.get("unhealthy_since") is None:
                self.state["unhealthy_since"] = datetime.utcnow().isoformat()
    ...
    def get_outage_duration(self) -> Optional[str]:
        ...
        if self.state.get("unhealthy_since"):
            try:
                ...
                duration = datetime.utcnow() - unhealthy_time
...
class HarveySelfHealingCheck:
    ...
    def run_checks(self):
        ...
        print_header("HARVEY ML SCHEDULER SANITY CHECK v5.0")
        print(f"\n{Colors.BLUE}Starting comprehensive system check with recovery detection...{Colors.ENDC}\n")
