...
# Fix Azure VM Environment and Configuration Issues

AZURE_VM_IP="20.81.210.213"
AZURE_USER="azureuser"

echo "=================================================="
echo "🔧 Fixing Azure VM Configuration Issues"
echo "=================================================="
echo ""

# Create comprehensive fix script
cat > fix_vm_environment.sh << 'EOF'
#!/bin/bash
...
echo "📤 Copying fix script to Azure VM..."
echo "You'll be prompted for password:"
scp fix_vm_environment.sh $AZURE_USER@$AZURE_VM_IP:/home/azureuser/harvey/

echo ""
echo "🔧 Running fix script on Azure VM..."
ssh $AZURE_USER@$AZURE_VM_IP "cd /home/azureuser/harvey && bash fix_vm_environment.sh"

echo ""
echo "🏃 Running sanity check to verify fixes..."
ssh $AZURE_USER@$AZURE_VM_IP "cd /home/azureuser/harvey && source .env && /home/azureuser/miniconda3/bin/python harvey_sanity_check_v3_with_email.py"
...
echo "  ✓ Environment variables now loaded from .env"
echo "  ✓ ML schedulers set up as cron jobs (not systemd timers)"
echo "  ✓ Sanity check updated to load environment"
echo "  ✓ Email alerts configured for dev@heydividend.com"
echo ""
