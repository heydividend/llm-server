...
class HarveyIntelligence:
    ...
    async def analyze_dividend(
        ...
        if enable_ensemble and query_type in [
            ...
            ai_response = await self._get_ai_response(provider, query, model_type)
    ...
    async def _ensemble_analysis(
        ...
        for model_type in models:
            provider = get_llm_provider(model_type)
            tasks.append(self._get_ai_response(provider, query, model_type))
    ...
    async def _get_ai_response(self, provider, query: str, model_type: ModelType) -> str:
        """Get AI response from a specific provider"""
        try:
            # For FinGPT (ML API), use ML integration instead
            if model_type == ModelType.FINGPT:
                return await self._get_fingpt_response(query)
            
            # For other models, use LLM provider
